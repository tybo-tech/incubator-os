<?php
include_once '../../config/Database.php';
include_once '../../models/User.php';
include_once '../../helpers/AuthGuard.php';
include_once '../../config/headers.php';
/**
 * GET → measures (metric types) that have at least one account binding.
 *
 * Read-only helper for the target popup measure-binding UI (Sprint 007 Phase 7).
 * Only measures backed by `metric_type_accounts` can be derived by
 * TargetMeasurementService, so the rest are intentionally omitted.
 */
try {
    $db = (new Database())->connect();
    auth_require_user($db);
    $stmt = $db->query("
        SELECT DISTINCT mt.id, mt.code, mt.name, mt.unit
        FROM metric_types mt
        JOIN metric_type_accounts mta ON mta.metric_type_id = mt.id
        ORDER BY mt.id ASC
    ");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    foreach ($rows as &$r) { $r['id'] = (int)$r['id']; }
    unset($r);
    echo json_encode($rows);
} catch (Throwable $e) { http_response_code(400); echo json_encode(['error'=>$e->getMessage()]); }
