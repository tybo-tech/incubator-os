...
@Component({
  selector: 'app-grant-seda-assessment',
  standalone: true,
  ...
  template: `
    <div class="p-6 space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-xl font-bold text-gray-900">SEDA Assessments</h2>
          <p class="text-sm text-gray-500">All companies — SEDA funding eligibility</p>
        </div>
      ...
      <div *ngIf="showImport()" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
        ...
        <p class="text-sm text-gray-500">Paste tab-separated data. First column is company ID, second is company name.</p>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr *ngFor="let item of records()" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 text-sm text-center">Level {{ item.data.level }}</td>
              ...
              <tr *ngIf="records().length === 0">
                <td colspan="12" class="px-4 py-8 text-center text-gray-500">No assessments found. Click "Import" to add data.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class GrantSedaAssessmentComponent implements OnInit {
  records = signal<INode<IBusinessAssessment>[]>([]);
  ...
  private resolveCompanyNames(records: INode<IBusinessAssessment>[]): void {
  ...
  runImport(): void {
    ...
    this.nodeService.addNodesBatch(nodes).subscribe({
      next: () => { this.importing.set(false); this.importResult.set({ success: true, message: `Imported ${nodes.length} assessment records.` }); this.importText = ''; this.loadAll(); },
