...
export class GrantSeedFundingComponent implements OnInit {
  ...
  runImport(): void {
    ...
    this.nodeService.addNodesBatch(nodes).subscribe({
      next: () => { this.importing.set(false); this.importResult.set({ success: true, message: `Imported ${nodes.length} seed funding records.` }); this.importText = ''; this.loadAll(); this.toast.success('Import complete.'); },
