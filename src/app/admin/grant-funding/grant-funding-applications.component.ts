    ...
    <div class="p-4 sm:p-6">
      ...
      >
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">
            Applicants not yet assigned to a client or cohort — pending
            qualification.
          </p>
        </div>
      ...
      <div class="flex flex-wrap gap-2 mb-4">
        ...
        >
          All
          <span
            class="bg-white/60 rounded-full px-1.5 py-0.5 text-[10px] font-bold leading-none tabular-nums"
          >
            {{ applications().length }}
          </span>
        </button>
          ...
          >
            ...
            >
              <path
                ...
                stroke-linejoin="round"
                stroke-width="2"
                d="M3 4h13M3 8h9m-9 4h9m5 4l-4 4m0 0l-4-4m4 4V8"
              />
            </svg>
          </button>
        </div>
      ...
      >
          ...
          <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th
                  class="text-left px-4 py-3 font-medium text-gray-600 hidden sm:table-cell"
                >
                  Reg. No
                </th>
                <th
                  class="text-left px-4 py-3 font-medium text-gray-600 hidden md:table-cell"
                >
                  Province
                </th>
                <th
                  class="text-left px-4 py-3 font-medium text-gray-600 hidden lg:table-cell"
                >
                  Turnover
                </th>
                <th class="text-left px-4 py-3 font-medium text-gray-600">
              ...
              >
                ...
                >
                  <div class="flex items-center space-x-3">
                    <div
                      class="w-8 h-8 bg-gradient-to-br from-green-400 to-teal-500 rounded-lg
                ...
                <td
                  class="px-4 py-3 hidden sm:table-cell text-gray-500 cursor-pointer"
                  (click)="openApplicant(app)"
                >
                  {{ app.data.registration_number || '—' }}
                </td>
                <td
                  class="px-4 py-3 hidden md:table-cell text-gray-500 cursor-pointer"
                  (click)="openApplicant(app)"
                >
                  {{ app.data.province || '—' }}
                </td>
                <td
                  class="px-4 py-3 hidden lg:table-cell cursor-pointer"
                  (click)="openApplicant(app)"
                >
                  <ng-container
                    *ngIf="
                      app.data.bank_statement_months ||
                        app.data.bank_statement_grand_total;
                      else noTurnover
                    "
                  >
                    <div class="flex items-center gap-1.5">
                      <span
                        class="inline-flex items-center px-2 py-0.5 rounded-md bg-teal-50 text-teal-700
        ...
        <div
          class="px-4 py-3 border-t border-gray-100 bg-gray-50 text-xs text-gray-500"
        >
        ...
        >
          ...
          <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">
              Move Applications to Stage
            </h3>
            <p class="text-sm text-gray-500 mt-1">
              Moving {{ selectedIds().size }} application(s) to a new workflow
              stage
            </p>
          </div>
...
export class GrantFundingApplicationsComponent implements OnInit {
  ...
  applyFilter(): void {
    ...
    list = [...list].sort((a, b) => {
      ...
      if (this.sortField === 'name') {
        cmp = (a.data.company_name ?? '').localeCompare(
          b.data.company_name ?? '',
        );
      } else {
        cmp =
          (a.data.bank_statement_grand_total ?? 0) -
          (b.data.bank_statement_grand_total ?? 0);
      }
  ...
  countHasTurnover(): number {
    return this.applications().filter(
      (a) => (a.data.bank_statement_grand_total ?? 0) > 0,
    ).length;
  }

  countHas12Months(): number {
    return this.applications().filter(
      (a) => (a.data.bank_statement_months ?? 0) >= 12,
    ).length;
  }
  ...
  turnoverToggleClass(active: boolean): string {
    const base =
      'inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border transition-colors cursor-pointer select-none ';
    return (
      base +
      (active
  ...
  pillClass(color: string, isActive: boolean): string {
    ...
    const map: Record<string, { off: string; on: string }> = {
      blue: {
        ...
        on: 'bg-blue-100 text-blue-800 border-blue-400 ring-1 ring-blue-400',
      },
      orange: {
        off: 'bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100',
        on: 'bg-orange-100 text-orange-800 border-orange-400 ring-1 ring-orange-400',
      },
      purple: {
        off: 'bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100',
        on: 'bg-purple-100 text-purple-800 border-purple-400 ring-1 ring-purple-400',
      },
      indigo: {
        off: 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100',
        on: 'bg-indigo-100 text-indigo-800 border-indigo-400 ring-1 ring-indigo-400',
      },
      green: {
        off: 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100',
        on: 'bg-green-100 text-green-800 border-green-400 ring-1 ring-green-400',
      },
      red: {
        off: 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100',
        on: 'bg-red-100 text-red-800 border-red-400 ring-1 ring-red-400',
      },
      teal: {
        off: 'bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100',
        on: 'bg-teal-100 text-teal-800 border-teal-400 ring-1 ring-teal-400',
      },
      pink: {
        off: 'bg-pink-50 text-pink-700 border-pink-200 hover:bg-pink-100',
        on: 'bg-pink-100 text-pink-800 border-pink-400 ring-1 ring-pink-400',
      },
      yellow: {
        off: 'bg-yellow-50 text-yellow-700 border-yellow-200 hover:bg-yellow-100',
        on: 'bg-yellow-100 text-yellow-800 border-yellow-400 ring-1 ring-yellow-400',
      },
      gray: {
        off: 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100',
        on: 'bg-gray-100 text-gray-800 border-gray-400 ring-1 ring-gray-400',
      },
    };
      ...
      .subscribe({
        next: (created) => {
          ...
          this.router.navigate([
            '/admin/grant-funding/applications',
            created.id,
            'overview',
          ]);
        },
  ...
  statusLabel(status?: string): string {
    return this.workflowSvc.getStatusLabel(
      this.workflowSvc.getWorkflow(this.WORKFLOW_ID),
      status,
    );
  }

  statusClass(status?: string): string {
    return this.workflowSvc.getStatusBadgeClass(
      this.workflowSvc.getWorkflow(this.WORKFLOW_ID),
      status,
    );
  }
  ...
  get allSelected(): boolean {
    ...
    return (
      ...
      this.filtered().every((app) => app.id && ids.has(app.id))
    );
  }
  ...
  get someSelected(): boolean {
    const ids = this.selectedIds();
    return (
      this.filtered().some((app) => app.id && ids.has(app.id)) &&
      !this.allSelected
    );
  }
  ...
  private finalizeBulkUpdate(completed: number, failed: number): void {
    ...
    if (failed === 0) {
      this.toastService.show(
        `${completed} application(s) moved successfully.`,
        'success',
      );
    } else {
      this.toastService.show(
