...
import {
  IndustryReportsService,
  IndustryDashboardResponse,
} from '../../../../services/industry-reports.service';
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-gray-600 text-sm mt-1">
            Key statistics across all industries
          </p>
        </div>
        <button
          (click)="refreshData()"
...
export class IndustryStatisticsComponent implements OnInit {
  ...
  industryDistributionChart = signal<IDoughnutChart | null>(null);

  constructor(
    private industryReportsService: IndustryReportsService,
  ...
  loadData(): void {
    ...
    this.industryReportsService
      .getDashboardStats()
      .pipe(
        catchError((error) => {
          console.error('Failed to load industry statistics:', error);
          this.error.set(
            'Failed to load industry statistics. Please try again.'
          );
      ...
      .subscribe((data: IndustryDashboardResponse) => {
        ...
        this.statsCards.set(
          this.industryReportsService.transformOverviewToStatsCards(
            data.overview
          )
        );
        ...
        // Generate chart data
        this.industryDistributionChart.set(
          this.chartHelper.getIndustryDistributionChart(data.top_industries)
        );
