          ...
          <div *ngFor="let program of filteredPrograms()"
            ...
            <div class="p-6">
              <div class="flex items-start justify-between mb-4">
                <h3 class="text-lg font-semibold text-gray-900 flex-1 pr-3">{{ program.name }}</h3>
                ...
                <div class="flex space-x-1 flex-shrink-0">
                  ...
                  <button
                    (click)="$event.stopPropagation(); deleteProgram(program)"
...
export class ProgramsListComponent implements OnInit {
  ...
  deleteProgram(program: Program): void {
    if (confirm(`Are you sure you want to delete "${program.name}" and all its cohorts? This action cannot be undone.`)) {
      this.isLoading.set(true);
      this.categoryService.deleteCategory(program.id).pipe(
        catchError(error => {
          console.error('Failed to delete program:', error);
          this.toastService.show('Failed to delete program. Please try again.', 'error');
          this.isLoading.set(false);
          return EMPTY;
        })
      ).subscribe(() => {
        this.toastService.show(`"${program.name}" deleted successfully.`, 'success');
        this.loadPrograms();
      });
    }
  }
