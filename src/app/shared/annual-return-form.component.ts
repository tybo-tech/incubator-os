...
@Component({
  selector: 'app-annual-return-form',
  standalone: true,
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-900">{{ isEdit() ? 'Edit' : 'New' }} Annual Return</h3>
        ...
        <div class="p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Year Ending</label>
            <input type="text" [(ngModel)]="form.yearEnding" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" placeholder="e.g., FY2026" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Anniversary Date</label>
              <input type="date" [(ngModel)]="form.anniversaryDate" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
            </div>
            <div>
          ...
          <div>
            ...
            <input type="date" [(ngModel)]="form.filingDate" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Fee Paid (R)</label>
              <input type="number" step="0.01" [(ngModel)]="form.feePaid" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
            </div>
          </div>
          <div>
...
export class AnnualReturnFormComponent implements OnInit {
  ...
  form: IAnnualReturn = {
    yearEnding: `FY${new Date().getFullYear()}`,
