      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr *ngFor="let item of sessions()" class="hover:bg-gray-50 transition-colors">
              <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ item.sessionDate }}</td>
              <td class="px-4 py-3 text-sm text-gray-700">{{ item.mentorName }}</td>
              ...
              <td class="px-4 py-3 text-center">
                <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full"
                  ...
                  [class.bg-yellow-100]="item.status === 'Rescheduled'"
                  [class.text-yellow-800]="item.status === 'Rescheduled'"
                  [class.bg-red-100]="item.status === 'Cancelled'"
                  [class.text-red-800]="item.status === 'Cancelled'">
                  {{ item.status }}
                </span>
              </td>
...
export class MentorshipListComponent {
  sessions = input.required<MentorshipSession[]>();
  view = output<MentorshipSession>();
  edit = output<MentorshipSession>();
  delete = output<MentorshipSession>();
}
