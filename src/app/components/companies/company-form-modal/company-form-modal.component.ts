...
import { ICompany, initCompany } from '../../../../models/simple.schema';
  ...
  template: `
    ...
    >
      ...
      >
              ...
              <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="flex items-center">
                    <input
                      [(ngModel)]="formData!.is_sars_registered"
                      name="is_sars_registered"
                      type="checkbox"
                      class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label class="ml-2 text-sm text-gray-700"
                      >SARS Registered</label
                    >
                  </div>

                  <div class="flex items-center">
                    <input
                      [(ngModel)]="formData!.has_tax_clearance"
                      name="has_tax_clearance"
                      type="checkbox"
                      class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label class="ml-2 text-sm text-gray-700"
                      >Has Tax Clearance</label
                    >
                  </div>

                  <div class="flex items-center">
                    <input
                      [(ngModel)]="formData!.has_cipc_registration"
                      name="has_cipc_registration"
                      type="checkbox"
                      class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label class="ml-2 text-sm text-gray-700"
                      >CIPC Registered</label
                    >
                  </div>

                  <div class="flex items-center">
                    <input
                      [(ngModel)]="formData!.has_valid_bbbbee"
                      name="has_valid_bbbbee"
                      type="checkbox"
                      class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label class="ml-2 text-sm text-gray-700"
                      >Valid BBBEE Certificate</label
                    >
                  </div>
                </div>
              </div>
        ...
        <!-- Modal Footer -->
        <div
          class="border-t border-gray-200 px-6 py-4 flex justify-end space-x-3"
        >
          <button
            type="button"
            (click)="onClose()"
...
export class CompanyFormModalComponent implements OnInit, OnChanges {
  ...
  @Output() companySaved = new EventEmitter<ICompany>();

  formData: ICompany = this.initCompany();
  saving = false;
  ...
  async onSave() {
    ...
    try {
      ...
      if (this.editMode && this.company) {
        ...
        savedCompany =
          (await this.nodeService.addCompany(newCompany).toPromise()) ||
          newCompany;
      }
