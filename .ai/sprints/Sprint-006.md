...
## Review — Sprint 006 continuation (2026-09-14 — Company 10 demo + deep UI verification) — commit `524b3c2`

**Demo dataset:** Company 10 (`Dlakadla Holdings`, `2014/155924/07`) seeded via real API endpoints only (no direct DB writes) — 1 current SWOT analysis (#13) with 8 findings (S2 W3 O2 T1), 6 GPS targets (finance 2 / strategy_general 2 / sales_marketing 1 / personal_development 1), 3 SWOT→GPS links with provenance, 3 targets left `legacy_unlinked`, tasks on target #49 (`progress_mode='tasks'`) + progress updates on 2 manual targets; `dashboard-counts` Total 6 · Overdue 1 · Due this month 4. Company 11 left intact.

**Defects found by verification, all fixed:**

1. **[HIGH] SWOT workspace linked-target hydration** — item card showed `1 GPS targets · 0 tasks` and expansion showed `Due — / Owner: — / 0% / No tasks yet` while the GPS screen showed 3 tasks @ 33.33%. `GpsTargetSource::listBySwotItem()` returned `gts.*` rows whose `id` is the **link row id** (not the target id) and omitted `due_date`/`owner_label`/`manual_progress_percentage`/`category`; the UI keyed `tasks()` off `t.id`, so tasks loaded for the wrong id. Fixed by selecting the full `gps_targets` row plus `gts.id AS source_id`, `link_notes`, `linked_at`, `swot_description`, `swot_category` — `id` is now the real target id, restoring symmetry with `listByTarget()`. `castRow()` casts the new numeric/float fields.
2. **[LOW] “Weaknesse” badge** — `q.label.slice(0,-1)` on `Weaknesses`; replaced with explicit `badge` on the `quadrants` config.
3. **[LOW] `NG0100 ExpressionChangedAfterItHasBeenCheckedError` every navigation** (`TabBarComponent`) — `#trigger` non-static `ViewChild` inside `*ngIf` bound to `[cdkConnectedOverlayOrigin]` flipped `undefined → ElementRef` after check. Fixed by moving the CDK overlay `<ng-template>` inside the `*ngIf` container so `trigger` resolves as a template-local ref.

**`doUnlink` hardened** to use `t.source_id` (the query-shape change made the previous `t.id`/`swot_item_id` heuristic unsafe — it could have deleted the wrong `gps_target_sources` row), with pair-based `unlinkByTargetAndSwot` fallback.

**Verification (Playwright, all pass):** expand weakness → `finance / Due 2026-08-20 / Owner: Financial Manager / in_progress / 33% / 3 tasks`; **add task** `33% → 25%` (1/4); **toggle complete** `25% → 50%` (2/4, backend `recalcTaskProgress`); **delete task** with confirm → `33%` (1/3); **unlink** → `Unlinked target #49` + empty state; **relink** via dropdown → `Linked target #49`, tasks/progress restored; GPS counts `Total 6 · Overdue 1 · Due this month 4`; overflow “More” shows legacy tabs; **0 console errors** on `overview` + `swot-v2`. `php -l` clean, `ng build` passes (budget warnings only).

**`nodes` accounting clarified:** `nodes` is dual-purpose (legacy archive **+ `activity_log` rows** in one table). Non-log node count **1901**; growth was `activity_log` page-view rows from QA navigation (user 77). Normalized writes never touch `nodes` (Company 10 analysis #13/targets 49–54 live only in `swot_analyses`/`gps_targets`). One non-log row added: `financial_indicator_request` id 4464 (Financial Indicators click during dropdown test).

**Committed:** `524b3c2` `fix(sprint-006): hydrate linked targets on SWOT workspace + tab-bar NG0100` (4 files, +45/−43) — `GpsTargetSource.php`, `swot-hierarchy.page.ts`, `tab-bar.component.ts`, `.gitignore`. Pushed `d44f1d4..524b3c2`.
